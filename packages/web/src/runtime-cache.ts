 ...
 *
 * Only one runtime is ever kept. Asset URLs carry the runtime version
 * (`<base>/<version>/wavewalletdk.wasm.gz`), so publishing a release changes
 * the key rather than overwriting the old one, and every load prunes whatever
 * it did not just ask for. A wallet that has been through several upgrades
 * therefore holds one runtime, not one per release it has ever seen.
 *
 ...
 *
 * The trailing version is the *schema* of what we store, not the runtime
 * version. Bump it when the stored representation changes so old buckets are
 * abandoned wholesale rather than misread; `openRuntimeCache` deletes any
 * sibling bucket sharing the prefix.
 */
export const RUNTIME_CACHE_NAME = 'wavelength-runtime-v1';
...
/**
 * pruneRuntimeCache deletes every entry other than `keepUrl`, which is how a
 * runtime from a previous release stops occupying disk.
 *
