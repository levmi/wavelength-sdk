...
Serve the compressed file as `Content-Type: application/wasm` with
`Content-Encoding: gzip` if your host lets you. That takes the fast path,
streaming compilation straight off the response, instead of inflating the module
in a buffer first.
